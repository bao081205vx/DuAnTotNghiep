...
(function(){
	...
	if(statusCountsRaw && Array.isArray(statusCountsRaw)){
		...
		for(const s of statusCountsRaw){
			...
			if(agg.has(norm)) agg.get(norm).count += cnt;
			else {
				// create display label (title case)
				const parts = rawLabel.trim().split(/\s+/).map(p => p.charAt(0).toUpperCase() + p.slice(1));
				agg.set(norm, { display: parts.join(' '), count: cnt });
			}
		}
