...
public interface HoaDonRepository extends JpaRepository<HoaDon, Integer> {
    ...
    @Query("select h from HoaDon h left join fetch h.khachHang left join fetch h.nhanVien left join fetch h.chiTietHoaDons cthd left join fetch cthd.sanPhamChiTiet spct where h.id = :id")
    HoaDon findByIdWithRelations(Integer id);

    // Fetch invoices for a specific customer, including key relations for list rendering
    @Query("select h from HoaDon h left join fetch h.khachHang left join fetch h.nhanVien left join fetch h.chiTietHoaDons cthd left join fetch cthd.sanPhamChiTiet spct where h.khachHang.id = :customerId order by h.ngayTao desc")
    java.util.List<HoaDon> findByKhachHangIdWithRelations(Integer customerId);
