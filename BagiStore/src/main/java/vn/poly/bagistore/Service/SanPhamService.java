...
public class SanPhamService {
    ...
    // Build product card DTOs with representative image and price range
    public List<vn.poly.bagistore.dto.ProductCardDTO> getProductCards() {
        List<SanPham> products = sanPhamRepository.findAll();
        List<SanPhamChiTiet> details = sanPhamChiTietRepository.findAll();

        Map<Integer, List<SanPhamChiTiet>> byProduct = details.stream()
                .filter(d -> d.getSanPham() != null && d.getSanPham().getId() != null)
                .collect(Collectors.groupingBy(d -> d.getSanPham().getId()));

        List<vn.poly.bagistore.dto.ProductCardDTO> out = new ArrayList<>();
        for (SanPham p : products) {
            List<SanPhamChiTiet> variants = byProduct.getOrDefault(p.getId(), new ArrayList<>());
            Double min = null, max = null;
            String image = null;
            for (SanPhamChiTiet v : variants) {
                Double g = v.getGia() != null ? v.getGia() : null;
                if (g != null) {
                    if (min == null || g < min) min = g;
                    if (max == null || g > max) max = g;
                }
                if (image == null && v.getAnhSanPham() != null && v.getAnhSanPham().getDuongDan() != null) {
                    image = v.getAnhSanPham().getDuongDan();
                }
            }
            // fallback placeholder image
            if (image == null) image = "/uploads/images/client1.jpg";
            // If no price found, set both to null
            out.add(new vn.poly.bagistore.dto.ProductCardDTO(p.getId(), p.getTenSanPham(), image, min, max));
        }
        return out;
    }
