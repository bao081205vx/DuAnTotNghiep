...
public class CustomerController {
    ...
    public ResponseEntity<?> updateCustomer(@PathVariable Integer id, @RequestBody CustomerCreateDTO payload) {
        ...
        if (payload.getUsername() != null && !payload.getUsername().trim().isEmpty()) {
            exist.setTenTaiKhoan(payload.getUsername());
        }
        if (payload.getPassword() != null && !payload.getPassword().trim().isEmpty()) {
            // encode password before storing
            exist.setMatKhau(passwordEncoder.encode(payload.getPassword()));
        }
    ...
    public ResponseEntity<?> createCustomer(@RequestBody CustomerCreateDTO payload) {
        ...
        // Set username and password from payload
        if (payload.getUsername() != null && !payload.getUsername().trim().isEmpty()) {
            kh.setTenTaiKhoan(payload.getUsername());
        }
        if (payload.getPassword() != null && !payload.getPassword().trim().isEmpty()) {
            // encode password before storing
            kh.setMatKhau(passwordEncoder.encode(payload.getPassword()));
        }

        // ensure non-nullable fields have defaults to avoid DB constraint errors
        if (kh.getMatKhau() == null || kh.getMatKhau().isEmpty()) kh.setMatKhau("");
