...
public class SanPhamController {
    ...
    public List<vn.poly.bagistore.dto.ProductCardDTO> getProductCards() {
        return sanPhamService.getProductCards();
    }
    ...
    public ResponseEntity<?> getProductById(@PathVariable Integer id) {
        try {
            ...
            for (ProductSummaryDTO s : summaries) if (s.getId().equals(id)) { found = s; break; }
    ...
    public ResponseEntity<?> createProductWithImages(@RequestPart("product") String productJson,
        ...
        try {
            ...
            // Return the created product object directly so the frontend (which expects result.id)
            // can immediately use the returned JSON. If you need to include productQr, the
            // frontend can construct the expected path or you can add a dedicated endpoint.
