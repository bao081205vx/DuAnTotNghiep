...
public class AuthController {
    ...
    public EmployeeResponseDTO login(@RequestBody Map<String, Object> payload){
        ...
        NhanVien e = opt.get();
        // Support both old SHA-256-stored passwords and new bcrypt-encoded passwords.
        String stored = e.getMatKhau();
        boolean valid = false;
        if (stored != null && stored.startsWith("$2")) {
            // bcrypt (or compatible) stored hash
            valid = passwordEncoder.matches(password, stored);
        } else {
            // fallback to legacy SHA-256 hex compare
            String hashed = hashPasswordSha256(password);
            valid = hashed.equalsIgnoreCase(stored == null ? "" : stored);
        }
        if (!valid) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid credentials");
        }
    ...
    public CustomerLoginResponse customerLogin(@RequestBody Map<String, Object> payload){
        ...
        KhachHang k = opt.get();
        String storedC = k.getMatKhau();
        boolean validC = false;
        if (storedC != null && storedC.startsWith("$2")) {
            validC = passwordEncoder.matches(password, storedC);
        } else {
            String hashedC = hashPasswordSha256(password);
            validC = hashedC.equalsIgnoreCase(storedC == null ? "" : storedC);
        }
        if (!validC) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid credentials");
        }
