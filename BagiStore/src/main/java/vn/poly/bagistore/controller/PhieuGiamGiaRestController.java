...
public class PhieuGiamGiaRestController {
    ...
    /**
     * Return all coupons applicable for a given order context (subtotal, optional productId/variantId).
     * This endpoint helps the frontend surface all eligible coupons for the current checkout.
     */
    @PostMapping("/applicable")
    public ResponseEntity<?> findApplicable(@RequestBody java.util.Map<String, Object> payload) {
        try {
            double subtotal = 0.0;
            if (payload.containsKey("subtotal") && payload.get("subtotal") != null) {
                try { subtotal = Double.parseDouble(String.valueOf(payload.get("subtotal"))); } catch(Exception ex) { subtotal = 0.0; }
            ...
            // optional product/variant ids (not used in current model but accepted for future use)
            Integer productId = null;
            Integer variantId = null;
            try { if (payload.containsKey("productId") && payload.get("productId") != null) productId = Integer.valueOf(String.valueOf(payload.get("productId"))); } catch(Exception ex) {}
            try { if (payload.containsKey("variantId") && payload.get("variantId") != null) variantId = Integer.valueOf(String.valueOf(payload.get("variantId"))); } catch(Exception ex) {}

            java.time.LocalDateTime now = java.time.LocalDateTime.now();
            List<PhieuGiamGia> list = service.findAll();

            List<java.util.Map<String,Object>> applicable = new java.util.ArrayList<>();
            for (PhieuGiamGia d : list) {
                if (d == null) continue;
                String status = d.getTrangThai();
                if (status == null) continue;
                String s = status.toLowerCase();
                if (!(s.contains("đang") || s.contains("active") || s.contains("hiện") || s.contains("true"))) continue;
                if (d.getNgayBatDau() != null && now.isBefore(d.getNgayBatDau())) continue;
                if (d.getNgayKetThu() != null && now.isAfter(d.getNgayKetThu())) continue;
                // check minimum order value
                if (d.getHoaDonToiThieu() != null && subtotal < d.getHoaDonToiThieu()) continue;
                // check stock/quantity
                if (d.getSoLuong() != null && d.getSoLuong() <= 0) continue;

                // passed checks -> include sanitized map
                java.util.Map<String, Object> m = new java.util.HashMap<>();
                m.put("id", d.getId());
                m.put("maPhieu", d.getMaPhieu());
                m.put("tenPhieu", d.getTenPhieu());
